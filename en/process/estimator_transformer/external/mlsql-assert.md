...
[mlsql 断言](https://github.com/allwefantasy/mlsql-plugins/tree/master/mlsql-assert) 插件提供了在表中使用 `assert` 断言命令，用于判断 MLSQL 的结果表中数据是否符合预期。
...
> 注意：示例中 mlsql 的 spark 版本为 2.4 ，如果需要在 spark 3.X 的版本运行，请将安装的插件设置为 `mlsql-assert-3.0`
